...
function setFarmCoords(type)
    local playerPed = PlayerPedId()
    local playerCoords = GetEntityCoords(playerPed)

    if type == 'recolte' then
        farmData.coords_recolte = playerCoords
        ESX.ShowNotification("Coordonnées de récolte définies.")
    elseif type == 'traitement' then
        farmData.coords_traitement = playerCoords
        ESX.ShowNotification("Coordonnées de traitement définies.")
    elseif type == 'vente' then
        farmData.coords_vente = playerCoords
        ESX.ShowNotification("Coordonnées de vente définies.")
    end
end
...
Citizen.CreateThread(function()
    while true do
        ...
        for _, farm in pairs(farmList) do
            ...
            if distanceTraitement < 10.0 then
